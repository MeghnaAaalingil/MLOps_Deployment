...
def convert_model(cfg):
    ...
    torch.onnx.export(
        ...
        # ONNX export is not training or evaluating the model. It's simply: Tracing the model's computation graph using one 
        # example input to generate a static representation of the model.
        # ONNX only needs to see the model's architecture and data flow:
            # Input shape
            # Data types
        ...
        dynamic_axes={
            "input_ids": {0: "batch_size"},  # This makes the exported ONNX model flexible to different batch sizes
